...
@section('content')
	...
	<div class="">
		<form action="/admin/product/edit/{{$datas->product_id}}" method="POST" enctype="multipart/form-data">
			@csrf
			<input type="hidden" class="form-control" name="product_id" id="product_id" value="{{$datas->product_id}}">
			<div class="row">	
				<div class="col-sm-7" style="padding: 10px; border: 1px solid #dedede;">
					<div class="form-group">
						...
						<select name="category_id" id="category_id" class="custom-select form-control">
							...
							<option 
							@if($datas->Catetory->category_catetory_id==$value->category_id)
                                {{"selected"}} 
                            @endif
					...
					<div class="form-group">
						...
						<select name="catetory_product_id" id="catetory_product_id" class="custom-select form-control">
							...
							<option 
							@if($datas->catetory_product_id==$value->catetory_id)
                                {{"selected"}} 
                            @endif
							value="{{$value->catetory_id}}">{{$value->catetory_name}}</option>
							@endforeach
						</select>
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="product_name" placeholder="Tên sản phẩm" name="product_name" value="{{$datas->product_name}}">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="product_price" placeholder="Giá bán" name="product_price" value="{{$datas->product_price}}">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="product_sale" placeholder="% Giảm giá(nếu có) ...." name="product_sale" value="{{$datas->product_sale}}">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="product_quantity" placeholder="Số lượng sản phẩm" name="product_quantity" value="{{$datas->product_quantity}}">
					...
					<div class="form-group">
						...
						<textarea name="product_content" id="product_content" class="form-control" cols="30" rows="3" placeholder="Nội dung về sản phẩm....">{{$datas->product_content}}</textarea>
					...
					<div class="form-group">
                        ...
                        <select name="product_status" class="form-control custom-select" id="product_status">
                            <option @if($datas->product_status==0) selected @endif value="0">Ẩn</option>
                            <option @if($datas->product_status==1) selected @endif value="1">Hiển thị</option>
                        </select>
				...
				<div class="col-sm-5" style="padding: 10px; border: 1px solid #dedede;border-left: 0;">
					<div class="form-group">
						<img src="/adminpages/images/{{$datas->product_image?$datas->product_image:'images_default.png'}}" alt="" style="width: 68%;height: 235px;" id="output_img">
					</div>
					<div class="form-group">
	                    <label for="product_image">Hình ảnh sản phẩm</label>
	                    <input type="file" name="product_image" class="form-control" id="product_image" placeholder="hình ảnh minh họa của sản phẩm...." value="{{$datas->product_image}}">
	                </div>
					<div class="form-group">
						<label for="product_desc">Mô tả:</label>
						<textarea name="product_desc" id="product_desc" class="form-control" cols="30" rows="3" placeholder="Mô tả ngắn gọn">{{$datas->product_desc}}</textarea>
					...
					<div class="form-group">
						...
						<select name="provider_product_id" id="provider_product_id" class="custom-select form-control">
							...
							<option 
							@if($datas->provider_product_id==$value->provider_id)
                                {{"selected"}} 
                            @endif
							value="{{$value->provider_id}}">{{$value->provider_name}}</option>
							@endforeach
						</select>
					...
					<div class="form-group">
						...
						<select name="brand_product_id" id="brand_product_id" class="custom-select form-control">
							...
							<option 
							@if($datas->brand_product_id==$value->brand_id)
                                {{"selected"}} 
                            @endif
							value="{{$value->brand_id}}">{{$value->brand_name}}</option>
							@endforeach
						</select>
